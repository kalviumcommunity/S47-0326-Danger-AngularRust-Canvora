-- Migration: Initial database schema for whiteboard application
...
CREATE TABLE IF NOT EXISTS draw_segments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    board_id UUID NOT NULL REFERENCES boards(id) ON DELETE CASCADE,
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_draw_segments_board_id ON draw_segments(board_id);
CREATE INDEX IF NOT EXISTS idx_draw_segments_created_at ON draw_segments(created_at);
